...
void main() {
  ...
  gl_FragColor = color;
      ...
      const updatePlacement = () => {
        ...
        renderer.dpr = Math.min(window.devicePixelRatio || 1, 1.35);
      ...
      const loop = (t) => {
        ...
        if (t - lastFrameRef.current < 1000 / 30) return;
      ...
      const startLoop = () => {
        if (!animationIdRef.current && activeRef.current && !document.hidden) {
      ...
      updatePlacement();
      const visibilityTarget = containerRef.current.parentElement || containerRef.current;
    ...
    const bootTarget = containerRef.current.parentElement || containerRef.current;
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      if (!activeRef.current || !containerRef.current || !rendererRef.current) return;
